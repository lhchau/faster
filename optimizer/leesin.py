...
class LEESIN(torch.optim.Optimizer):
    def __init__(self, params, rho=0.05, adaptive=False, top=0.2, alpha=2, **kwargs):
        assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"

        defaults = dict(rho=rho, adaptive=adaptive, **kwargs)
        super(LEESIN, self).__init__(params, defaults)
        self.top = top
        self.alpha = alpha
    ...
    def first_step(self, zero_grad=False):   
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                threshold = torch.quantile(p.grad.view(-1).abs(), 1-self.top)
                param_state['mask'] = p.grad.abs() >= threshold
                d_t = p.grad.mul(torch.logical_not(param_state['mask'])) + p.grad.mul(param_state['mask']).mul(self.alpha)
                e_w = (torch.pow(p, 2) if group["adaptive"] else 1.0) * d_t * scale
