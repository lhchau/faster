...
class SAMADAM(torch.optim.Optimizer):
    def __init__(self, params, rho=0.05, adaptive=False, beta=(0.9, 0.995), **kwargs):
        assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"

        defaults = dict(rho=rho, adaptive=adaptive, **kwargs)
        super(SAMADAM, self).__init__(params, defaults)
    ...
    def second_step(self, zero_grad=False):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                param_state['exp_avg_sq'].mul_(self.beta2).addcmul_(d_p, d_p.conj(), value=1 - self.beta2)
                
                denom = (param_state['exp_avg_sq'].sqrt() / math.sqrt(bias_correction2)).add_(1e-6)
