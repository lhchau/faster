...
def generate_data(n_samples=200, noise_rate=0):
...
X_noise, y_noise = generate_data(noise_rate=0.2)
...
class LinearModel(nn.Module):
    def __init__(self):
        ...
        self.linear2.weight.data = torch.tensor([[-4, -4]], dtype=torch.float32)
        # self.linear2.bias.data = torch.tensor([0.0], dtype=torch.float32)
...
def train_model_sgd(train_loader):
    ...
    trajectory = [model.linear2.weight.data.clone().cpu().numpy().flatten()]
    ...
    for epoch in range(EPOCHS):
        for X_batch, y_batch in train_loader:
            ...
            loss.backward()
            optimizer.step()
            trajectory.append(model.linear2.weight.data.clone().cpu().numpy().flatten())
...
model_clean, trajectory_clean = train_model(train_loader, opt_name, log_groupB=True)
...
def visualize_loss_landscape(model, X, y, trajectory, title, filename):
    ...
    for i in range(W1.shape[0]):
        for j in range(W1.shape[1]):
            model.linear2.weight.data = torch.tensor([[W1[i, j], W2[i, j]]], dtype=torch.float32)
...
visualize_loss_landscape(model_clean, X, y, trajectory_clean, "Loss Landscape (Clean Data)", f"MLP_loss_landscape_clean_{opt_name}.pdf")
visualize_loss_landscape(model_noisy, X_noise, y_noise, trajectory_noisy, "Loss Landscape (Noisy Data)", f"MLP_loss_landscape_noisy_{opt_name}.pdf")
